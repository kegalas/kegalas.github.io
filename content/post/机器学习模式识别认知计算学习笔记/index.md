...
由于$w$并非沿着$\mu_i-\mu)j$方向，因此分界面并非与均值间的连线垂直正交。
...
- 线性核函数$\kappa(x+i,x_j)=x_i^Tx_j$
- 多项式核函数$(x_i^Tx_j)^d$，其中$d\geq 1$为多项式的次数。也有写作$(sx_i^Tx_j+c)^d$的。
- 径向基核函数$\exp(-\gamma||x_i-x_j||^2)$。$\gamma=1/2\sigma^2$时称为高斯核，$\gamma=\sigma$时称为拉普拉斯核
...
在实际应用中，往往单个点分布的概率都很小，为了便于求解， 一 般 对似然函数取对数 ，得到对数似然函数。对高斯混合模型求对数似然函数
...
根据矩阵边矩阵$W$建立一个标记传播矩阵$S = D^{-1/2}WD^{-1/2}$。其中$D=diag(\dfrac{1}{\sqrt{d_1}}, \dfrac{1}{\sqrt{d_2}}, \cdots, \dfrac{1}{\sqrt{d_{l+u}}})$，于是我们得到迭代公式
...
从全体特征开始，每次剔除一个特征，使得保留的特征集合具有最大的可分性或分类识别率。依次迭代，知道可分性或识别率开始下降。

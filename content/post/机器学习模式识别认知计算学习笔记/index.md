...
其中$y(x,w)=w^Tx$是拟合出来的线性模型，$\epsilon$符合均值为$0$的高斯分布，其精确度为$\beta$。精确度即为方差的导数。显然，可以知道$E(t)=E(y)=w^Tx, Var(t)=Var(\epsilon)=\sigma^2=\beta^{-1}$，我们可以记作
...
考虑整个训练集，原样本点$x_i$与机遇投影重构的样本点$\hat x_i$之间的距离为
...
为了分析有向图中变量间的条件独立性，可以使用有向分离（D-separation）。先把有向图转位无向图
